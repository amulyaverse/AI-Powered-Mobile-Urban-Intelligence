...
export default function TrafficAnalytics() {
  ...
  const loadData = useCallback(async (isInitial = false) => {
    ...
    try {
      ...
      console.error('[TrafficAnalytics] Failed to load traffic analytics:', err);
  ...
  if (loading && !data) {
    return <LoadingState message="Loading traffic intelligence and density models..." />;
  }

  if (error && !data) {
    return (
      <ErrorState
        title="Traffic Analytics Unavailable"
        message="Could not load traffic telemetry charts. You can retry the connection or switch to Demo Mode."
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Density Over Time */}
        <div className="lg:col-span-2 bg-white p-5 rounded-lg shadow-xs border border-slate-200">
        ...
        {/* Vehicle Types */}
        <div className="bg-white p-5 rounded-lg shadow-xs border border-slate-200 flex flex-col">
        ...
        {/* Route Analytics */}
        <div className="lg:col-span-3 bg-white p-5 rounded-lg shadow-xs border border-slate-200">
              ...
              <div key={route.id} className="p-4 rounded-lg border border-slate-100 bg-slate-50 flex flex-col gap-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className={`px-2 py-0.5 rounded text-xs font-bold ${
                    route.density === 'HIGH' || route.density === 'CRITICAL' ? 'bg-red-100 text-red-700' :
